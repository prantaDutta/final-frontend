define('./workbox-ea903bce.js', ['exports'], function (t) {
  ...
  class a {
    ...
    handleRequest({ request: t, event: e }) {
      ...
      const n = s.origin === location.origin,
        { params: i, route: r } = this.findMatchingRoute({ event: e, request: t, sameOrigin: n, url: s })
      let a = r && r.handler
      const c = t.method
  ...
  class v {
    ...
    async fetch(t) {
      ...
      try {
        ...
        for (const s of this.iterateCallbacks('fetchDidSucceed')) t = await s({ event: e, request: r, response: t })
        return t
      } catch (t) {
        throw (
          (i &&
    ...
    async cacheMatch(t) {
      ...
      for (const t of this.iterateCallbacks('cachedResponseWillBeUsed'))
        s = (await t({ cacheName: n, matchOptions: i, cachedResponse: s, request: r, event: this.event })) || void 0
      return s
    }
    async cachePut(t, e) {
      const n = R(t)
      ...
      for (const t of this.iterateCallbacks('cacheDidUpdate'))
        await t({ cacheName: o, oldResponse: f, newResponse: c.clone(), request: r, event: this.event })
      return !0
    }
  ...
  class q {
    ...
    async v(t, e, n) {
      ...
      try {
        if (((i = await this.U(e, t)), !i || 'error' === i.type)) throw new s('no-response', { url: e.url })
      } catch (s) {
        for (const r of t.iterateCallbacks('handlerDidError'))
          if (((i = await r({ error: s, event: n, request: e })), i)) break
        if (!i) throw s
      }
      for (const s of t.iterateCallbacks('handlerWillRespond')) i = await s({ event: n, request: e, response: i })
      return i
    }
    async q(t, e, s, n) {
      let i, r
      ...
      try {
        await e.runCallbacks('handlerDidRespond', { event: n, request: s, response: i }), await e.doneWaiting()
      } catch (t) {
        r = t
      }
      if ((await e.runCallbacks('handlerDidComplete', { event: n, request: s, response: i, error: r }), e.destroy(), r))
        throw r
    }
  }
  function U(t) {
  ...
  class x {
    constructor(t, e, { onupgradeneeded: s, onversionchange: n } = {}) {
      ;(this._ = null), (this.L = t), (this.N = e), (this.C = s), (this.D = n || (() => this.close()))
    }
    get db() {
      return this._
    }
      ...
      if (!this._)
        return (
          (this._ = await new Promise((t, e) => {
            ...
            ;(n.onerror = () => e(n.error)),
              (n.onupgradeneeded = (t) => {
                s ? (n.transaction.abort(), n.result.close()) : 'function' == typeof this.C && this.C(t)
              }),
              (n.onsuccess = () => {
                const e = n.result
    ...
    async getAllKeys(t, e, s) {
      return (await this.getAllMatching(t, { query: e, count: s, includeKeys: !0 })).map((t) => t.key)
    }
    async getAllMatching(t, { index: e, query: s = null, direction: n = 'next', count: i, includeKeys: r = !1 } = {}) {
  ...
  class C {
    constructor(t) {
      ;(this.P = t), (this._ = new x('workbox-expiration', 1, { onupgradeneeded: (t) => this.k(t) }))
    }
    k(t) {
    ...
    async expireEntries(t, e) {
      const s = await this._.transaction(L, 'readwrite', (s, n) => {
          ...
          i.onsuccess = () => {
            ...
            if (s) {
              const n = s.value
              n.cacheName === this.P && ((t && n.timestamp < t) || (e && a >= e) ? r.push(s.value) : a++), s.continue()
            } else n(r)
          }
        }),
        n = []
  ...
  class A {
    ...
    addToCacheList(t) {
      ...
      for (const n of t) {
        ...
        if (this.G.has(i) && this.G.get(i) !== t)
          throw new s('add-to-cache-list-conflicting-entries', { firstEntry: this.G.get(i), secondEntry: t })
    ...
    install(t) {
      return D(t, async () => {
        ...
        for (const [e, s] of this.G) {
          ...
          await Promise.all(this.strategy.handleAll({ params: { cacheKey: s }, request: r, event: t }))
        }
    ...
    createHandlerBoundToURL(t) {
      ...
      return (s) => ((s.request = new Request(t)), (s.params = p({ cacheKey: e }, s.params)), this.strategy.handle(s))
    }
  }
  const S = () => (O || (O = new A()), O)
  class I extends i {
  ...
  ;(t.CacheFirst = class extends q {
    ...
    (t.NetworkFirst = class extends q {
      ...
      async U(t, e) {
        ...
        i.push(a)
        const c = await e.waitUntil((async () => (await e.waitUntil(Promise.race(i))) || (await a))())
