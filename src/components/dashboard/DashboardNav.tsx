            ...
            <div className="p-4 relative flex items-center">
                ...
                {data && (
                    <>
                        {data?.count > 0 && (
                            <div
                                className="bg-primary text-white text-xs font-bold rounded-full px-1.5 py-0.5 absolute z-10 top-0 left-1/2 mt-1">
                                <span>{data?.count}</span>
                            </div>
                        )}
                        {showNotificationsDiv && (
                            <div
                                className="mt-2 py-2 w-max bg-white rounded-lg shadow-xl absolute top-full transform -translate-x-72 z-10">
                                {data.notifications.length > 0 ? (
                                    data.notifications.map((notification: any, i: number) => {
                                        return (
                                            <div key={notification.id}>
                                                <div
                                                    className="flex flex-grow justify-start items-center px-4 py-2 text-gray-800 text-sm font-bold">
                                                    <svg
                                                        className="w-6 h-6"
                                                        fill="none"
                                                        stroke="currentColor"
                                    ...
                                    <>
                                        <div
                                            className="flex justify-start items-start px-4 py-2 text-gray-800 text-sm font-bold">
                                            <svg
