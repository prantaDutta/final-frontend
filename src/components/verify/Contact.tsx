    ...
    resolver: yupResolver(
      object({
        ...
        mobileNo: yup
          ...
          .test('Mobile No Verified', 'Mobile No. Not Verified', async function () {
            try {
              const { data } = await laravelApi().get('/user/contact-verified')
      ...
      <main className="bg-white max-w-full mx-auto p-4 md:p-8 my-5 rounded-lg shadow-2xl">
        ...
        <form className="mt-5" onSubmit={handleSubmit(onSubmit)}>
          <div className="md:flex items-end px-4">
            <InputTextField
              halfWidth
              defaultValue={verificationValues?.email ? verificationValues?.email : userData ? userData.email : ''}
              name="email"
              label="Your Email"
              ...
              register={register}
            />
            <button className={`chip p-1.5 bg-primary mx-4 mb-2 cursor-not-allowed`}>
              {contactData?.email ? 'verified' : 'unverified'}
            </button>

            <div className="flex">
              <button type="button" className="chip p-1.5 bg-primaryAccent mb-2 mx-4" onClick={() => setEmailOtp(true)}>
                Enter Otp
              </button>
              <button
                type="button"
                ...
                className={`chip ${emailSending ? 'p-0' : 'p-1.5'} mx-4 mb-2 bg-primaryAccent`}
              >
          ...
          {showEmailOtp && (
            <div className="max-w-md px-8 pt-4">
              <label className="text-md font-bold text-gray-700 tracking-wide">Enter OTP From Your Email</label>
              <input
          ...
          <div className={`md:flex items-end px-4`}>
            ...
            <button
              onClick={async () => {
                ...
                if (!errors.mobileNo || errors.mobileNo.message === 'Mobile No. Not Verified') {
              ...
              type="button"
              className={`chip ${smsSending ? 'p-0' : 'p-1.5'} mx-4 mb-2 bg-primaryAccent`}
            >
          ...
          {showMobileOtp && (
            <div className="max-w-md px-8 pt-4">
              <label className="text-md font-bold text-gray-700 tracking-wide">Enter OTP From Your Mobile</label>
              <input
                className="w-full bg-white font-semibold pl-5 my-2 rounded-lg py-2 border-b border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary"
