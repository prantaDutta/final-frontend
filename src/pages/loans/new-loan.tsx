...
const NewLoan: React.FC<newLoanProps> = ({ user }) => {
      ...
      Yup.object({
        ...
        monthlyInstallment: Yup.number()
          ...
          .test(
            ...
            'Something is Wrong',
            (value) => value == modifiedMonthlyInstallment || value == newFormState?.monthlyInstallment
          )
          .required('Required')
      })
    ),
  ...
  const submitHandler = (values: NewLoanFormValues) => {
    ...
    let modifiedInstallment = formatTwoDecimalPlaces(calculateSimpleInterest(+amount, +interestRate + 2, +loanDuration))
    setFormState({
      ...values,
  ...
  return (
    <DashboardLayout data={user} title={`Apply For A New Loan`}>
      ...
      {user.verified === 'verified' ? (
        complete ? (
          <div className="px-4">
            <p className="text-2xl font-bold pl-5 mt-8">Thank You for Submitting.</p>
            <div className="p-5">
              <p className="text-xl font-bold mt-5">Loan Amount: {newFormState?.amount} Tk.</p>
              <p className="text-xl font-bold mt-5">Interest Rate: {newFormState?.interestRate}%</p>
              <p className="text-xl font-bold mt-5">Loan Duration: {newFormState?.loanDuration} Months</p>
              ...
              <div className="mt-5">
                ...
                <button
                  onClick={async () => {
                    ...
                    setSubmitting(true)
                    if (!isProduction) console.log('Sending Values: ', newFormState)
                    try {
                  ...
                  shadow-lg transition-css"
                >
                  {submitting ? <ReactLoader component={<ThreeDots width="50" />} /> : 'Proceed'}
                </button>
              </div>
            </div>
          </div>
          ...
          <main className="bg-white w-full mx-auto p-4 md:p-8 mt-5 rounded-lg shadow-2xl">
            <p className="pt-2 pl-8 text-xl font-semibold">Enter the following Data</p>
            <form onSubmit={handleSubmit(submitHandler)}>
              ...
              <div className="flex px-4">
                <InputSelectField
                  ...
                  defaultValue={newFormState ? newFormState.loanDuration : undefined}
                ...
                <InputTextField
                  ...
                  register={register}
                  value={
                    modifiedMonthlyInstallment ? modifiedMonthlyInstallment : newFormState?.monthlyInstallment || ''
                  }
